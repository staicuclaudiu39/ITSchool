...
const finalListExample = [{
fullName: "Valeria Murgoci",
position: "Design Lead",
isTeamLead: true,
phoneNumber: 2722345834,
email: "valeria.murg@camex.ro",
address: "Bucuresti, Soseaua Pipera, Nr 13",
photoUrl: "https://i1.sndcdn.com/artworks-000021223010-8s64rf-t500x500.jpg"
}]
...
for (let i = 0; i < memberList.length; i++) {
    if (memberList[i].address.city != 'Galati') {
        tempObj.fullName = memberList[i].name + " " + memberList[i].surname;
        tempObj.position = memberList[i].positon;
        tempObj.isTeamLead = memberList[i].isTeamLead;
        tempObj.phoneNumber = parseInt(memberList[i].phoneNumber);
        tempObj.email = memberList[i].emailAddress;
        tempObj.address = memberList[i].address.city + " " + memberList[i].address.street + " " + memberList[i].address.nr;

        for (let j = 0; j < photoList.length; j++) {
            if (photoList[j].userId == memberList[i].userId)
                tempObj.photoUrl = photoList[j].photoUrl;
        }
        
        // tempObj trebuie dereferentiat, asa ca am mai creat un object care sa faca asta.
